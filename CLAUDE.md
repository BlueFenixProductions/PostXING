...
- **Editor bridge is the sharp edge.** MAUI's `HybridWebView` renders `Resources/Raw/editor/index.html`, but its **JS↔host raw-message bridge does not work on Android** in MAUI 10.0.20: `window.HybridWebView` is never injected, so `SendRawMessage`/`RawMessageReceived` never fire. And `EvaluateJavaScriptAsync`'s awaited Task **hangs during page load** (its *return value* works once the page is stable). So `EditorPage` (`#if ANDROID`):
  ...
  - **JS→host (dirty-tracking, gh #29):** the dead bridge also means a keystroke can't push a `change`, so on its own `IsDirty` never flips on Android and Save/Publish stay disabled (grayed in the overflow). `EditorPage` runs an `IDispatcherTimer` (~750ms, started in `OnAppearing`, stopped in `OnDisappearing`) that pulls `getText()` the same way and mirrors it into `RawMarkdown` when it diverges — which flips `IsDirty` (and keeps the word-count live). The poll refuses to let a spurious empty read wipe a non-empty doc; an explicit Save still honors an intentionally emptied editor.
